...
const Search = () => {
  ...
  const handleSelect = async (result: SearchResult) => {
    ...
    try {
      const res = await fetch(`/api/scrape?url=${encodeURIComponent(result.link)}`);
      ...
      {selectedResult ? (
        <div className="bg-white p-6 rounded-lg shadow-md">
          {isLoading ? (
            ...
            <>
              <h3 className="text-2xl font-bold mb-4">{productDetails.productName}</h3>
            ...
            <p className="text-center text-gray-500">Failed to load product details.</p>
          )}
        </div>
      ) : (
        <ul className="space-y-4">
          {results.length > 0 ? (
            results.map((result, index) => (
              ...
              >
                <h4 className="text-xl font-semibold mb-2">{result.title}</h4>
